...
class NeuMFBaseline(BaseBaseline):
    def fit(self,
            ...
            *,
            ...
            epochs=None, batch=None, n_neg=4) -> tuple[dict, list[dict]]:

        cfg     = self.cfg
        mf_dim  = mf_dim  or getattr(cfg, "neumf_mf_dim", 16)
        mlp_dim = mlp_dim or getattr(cfg, "neumf_mlp_dim", 32)
        epochs  = epochs  or getattr(cfg, "neumf_epochs",   15)
        batch   = batch   or getattr(cfg, "neumf_batch",  1024)
        lr      = getattr(cfg, "neumf_lr", 1e-3)

        self.item_meta = (
            train_df.drop_duplicates(item_col)
                    .set_index(item_col)[["title", "genres"]]
                    .to_dict("index")
            if {"title", "genres"}.issubset(train_df.columns) else {}
        )

        pos = train_df[train_df[reward_col] == 1].copy()
        keep_u = pos[user_col].value_counts()
        pos = pos[pos[user_col].isin(keep_u[keep_u >= min_inter].index)]
        ...
        user_pos = pos.groupby("u_idx")["i_idx"].agg(list).to_dict()
        ...
        for ep in range(1, epochs + 1):
            ...
            print(f"[NeuMF] epoch {ep:02d} | loss = {mean_loss:.4f}")

        metrics = {"final_train_loss": self._loss_history[-1]}

        return metrics, []

    @torch.inference_mode()
    def rank(self, user_ids: torch.Tensor, k: int | None = None) -> torch.Tensor:
        known = [u.item() for u in user_ids if u.item() in self.user_map]
        if not known:
            k = k or 0
            return torch.empty((0, k), dtype=torch.long)

        u_idx = torch.tensor([self.user_map[u] for u in known], device=self.device)
        scores = self.model(
        ...
        return top_idx.cpu().apply_(lambda x: self.inv_item_map[int(x)])
    ...
    def predict_sequences(self,
        ...
        """
        TOP‑k pour chaque user du **jeu test**.
        """
        self.model.eval()
        logs: list[dict] = []

        for u_raw in sorted(test_df["user_id"].unique()):
